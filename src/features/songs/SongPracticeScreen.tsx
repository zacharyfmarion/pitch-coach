...
import { createSongDebugInfo, createVocalEnergyTrace } from "../../song/debugDiagnostics";
import { formatSongReferenceRange } from "../../song/referenceRange";
import { SongPitchTimeline } from "../../song/SongPitchTimeline";
import {
  SONG_SECTION_LIMITS,
  useSongPracticeController,
  type SongPracticeController
} from "../../song/useSongPracticeController";
...
export function SongPracticeScreen({ services, onBackToLibrary }: SongPracticeScreenProps) {
  ...
  const hasPracticeReference = Boolean(song.reference);
    ...
    <main className="app-shell">
      <section className="coach-workspace" aria-label="Song practice">
        ...
        <section className="practice-layout song-practice-layout">
          <div className={`lesson-panel song-workbench${hasPracticeReference ? "" : " song-workbench--prepare"}`}>
            {hasPracticeReference || song.stage === "unsupported" ? (
              <SongPitchTimeline
                reference={song.reference}
              ...
              <SongPrepareWorkbench song={song} />
            )}

            {!hasPracticeReference && song.stage !== "unsupported" ? (
              <div className="song-prep-timeline-preview">
                <SongPitchTimeline
                  reference={song.reference}
                  liveFrames={song.liveFrames}
                  score={song.score}
          ...
          <aside className="side-panel song-side-panel" aria-label="Song controls and feedback">
            {song.stage === "unsupported" ? (
              <UnsupportedSongPanel song={song} />
            ) : (
              <>
                <SongAnalysisPanel
                  song={song}
                ...
                {debugEnabled && song.reference && debugInfo ? (
                  <SongDebugPanel debugInfo={debugInfo} />
                ) : null}
                {song.reference ? <SongPracticeMixPanel song={song} /> : null}
                <SongFeedbackPanel song={song} />
              </>
            )}
          </aside>
        </section>
...
function SongPrepareWorkbench({ song }: { song: SongPracticeController }) {
  return (
    <section className="song-prepare-workbench" aria-label="Prepare song section">
      <div className="song-prepare-workbench__header">
        <span className="brand-mark" aria-hidden="true">
          <Upload size={22} />
        </span>
        <div>
          <span className="readout-label">Prepare</span>
          <h2>{song.fileName ?? "Choose Audio"}</h2>
        </div>
      </div>

      <label className="song-upload-target">
        <span>Audio</span>
        <input
          type="file"
          accept="audio/*"
          ...
          disabled={song.isBusy || song.stage === "checking"}
        />
      </label>

      {song.decodedAudio ? (
        <div className="song-section-controls">
          <label>
            <span>Start</span>
            <input
              type="range"
...
function UnsupportedSongPanel({ song }: { song: SongPracticeController }) {
  return (
...
function SongAnalysisPanel({
  song,
  debugEnabled,
  setDebugEnabled
}: {
  song: SongPracticeController;
  ...
  return (
    <section className="control-group" aria-label="Song analysis">
      ...
      {song.reference ? (
        <details className="song-debug-disclosure">
...
function SongDebugPanel({ debugInfo }: { debugInfo: NonNullable<ReturnType<typeof createSongDebugInfo>> }) {
  return (
...
function SongPracticeMixPanel({ song }: { song: SongPracticeController }) {
  return (
...
function SongFeedbackPanel({ song }: { song: SongPracticeController }) {
  return (
